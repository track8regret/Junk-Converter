...
export async function searchDLMByID(id: number): Promise<Card> {
    var response: Response = await fetch('https://www.duellinksmeta.com/api/v1/cards?id=' + encodeURIComponent(id)).then(e => e.json()) as Response;
    var noalts = response.filter(card => !card.alternateArt)
    if (noalts.every(card => card.popRank === Number.MAX_VALUE)) {
        return noalts[0]
    } else {
...
export async function searchMDMByID(id: number): Promise<Card> {
    var response: Response = await fetch('https://www.masterduelmeta.com/api/v1/cards?name=' + encodeURIComponent(id)).then(e => e.json()) as Response;
    var noalts = response.filter(card => !card.alternateArt)
    if (noalts.every(card => card.popRank === Number.MAX_VALUE)) {
        return noalts[0]
    } else {
