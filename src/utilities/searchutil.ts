import { CommandContext, MessageEmbedOptions, EmbedField, Message } from 'slash-create';
...
import { RushCardIO, Card as RIOCard } from '../utilities/rushcardio.js'
...
export async function searchCard (query: string, ctx: CommandContext) {
    switch (ctx.commandName) {
            ...
            if ((cardinfo.card_sets) || (dlcard && dlcard.rarity) || (mdcard && mdcard.rarity)) {
                let settext = '';
                let dltext = '';
                let mdtext = '';
            ...
            var embed: MessageEmbedOptions = {
                ...
                color: embedcolor,
                thumbnail: {
                    url: (cardinfo.card_images[0].image_url_cropped ? cardinfo.card_images[0].image_url_cropped : cardinfo.card_images[0].image_url)
                },
            ...
            if (!isNaN(Number(query))) {
                ...
                rushinfo = await RIO.searchCard(query)
            }
            if (rushinfo === undefined) {
                // do something, exit
                return ctx.send({content: 'The card you searched for turned up as undefined in our search.\nUsually, this happens when RushCard doesn\'t have information on a card we list.\nIf you suspect it\'s something else, please try again.', ephemeral: true})
            }

            let rushfields: Array<EmbedField> = [];

            if (rushinfo.desc.includes('[REQUIREMENT]')) {
                var fuckypd = rushinfo.desc.replace(/[:;]+(?!\ )/g, '')
                var requirement = fuckypd.split('[REQUIREMENT]')[1].split('[')[0]
                rushfields.push({name: 'Requirement', value: requirement})
                if (fuckypd.includes('[EFFECT]')) {
                    var effect = fuckypd.split('[EFFECT]')[1]
                    rushfields.push({name: 'Effect', value: effect})
                }
                if (fuckypd.includes('[MULTI-CHOICE EFFECT]')) {
                    var effect = fuckypd.split('[MULTI-CHOICE EFFECT]')[1]
                    rushfields.push({name: 'Possible Effects', value: effect})
                }
            }

            let rushcolor = 0x000000;
            let rushlevel: string = '';

            const rushmatchers: Array<[(card: RIOCard) => boolean, number]> = [
                [rushinfo => rushinfo.type.includes('Link'), 0x0685CC],
                [rushinfo => rushinfo.type.includes('XYZ'), 0x161616],
                [rushinfo => rushinfo.type.includes('Synchro'), 0xE7E6E4],
                [rushinfo => rushinfo.type.includes('Fusion'), 0x8E3E9D],
                [rushinfo => rushinfo.type.includes('Ritual'), 0x476FB5],
                [rushinfo => rushinfo.type.includes('Pendulum'), 0x5CB8AD],
                [rushinfo => rushinfo.type.includes('Effect'), 0xC75227],
                [rushinfo => rushinfo.type.includes('Spell'), 0x008B78],
                [rushinfo => rushinfo.type.includes('Trap'), 0xA5146F],
                [rushinfo => rushinfo.type.includes('Normal'), 0xCC9A53]
            ];

            for (const [matcher, color] of rushmatchers) {
                if (matcher(rushinfo)) {
                    rushcolor = color;
                    break;
                }
            ...
            var rushembed: MessageEmbedOptions = {
                ...
                title: rushinfo.name,
                url: 'https://yugipedia.com/wiki/' + encodeURIComponent(rushinfo.id),
                color: rushcolor,
                thumbnail: {
                    url: 'https://images.rushcard.io/images/card/' + rushinfo.id + '.jpg'
                },
